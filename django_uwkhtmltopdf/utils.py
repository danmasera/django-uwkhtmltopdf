...
import StringIO
import os
from options import supported_options 
...
def parse_options(options, context):
    ...
    opts = []
    for opt in options:
        if opt in supported_options['no_parameter']:
            opts.extend(['--' + opt ])
        elif opt in supported_options['one_parameter']:
            opts.extend(['--' + opt , options[opt]])
        elif opt in supported_options['two_parameters']:
            opts.extend(['--' + opt , options[opt][0] , options[opt][1]])
        elif opt in supported_options['template_needed']:
            head_or_foot_template = render_to_tmp_file(options[opt], context)
            opts.extend(['--' + opt, head_or_foot_template])
            files_to_remove.append(head_or_foot_template)
